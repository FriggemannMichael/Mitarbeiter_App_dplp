...
if (!customerIdRaw) {
  fail("Usage: npm run dev:customer -- <customer-id>");
}
...
const args = isNpmScriptContext ? [npmExecPath, "run", "dev"] : ["run", "dev"];

const result = spawnSync(command, args, { stdio: "inherit", env });

if (result.error) {
  fail(`Customer dev server failed: ${result.error.message}`);
}

if (result.status !== 0) {
  fail("Customer dev server failed.", result.status ?? 1);
}
